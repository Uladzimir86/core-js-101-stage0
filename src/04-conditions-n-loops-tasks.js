...
function isInsideCircle(circle, point) {
  ...
  const x = Math.abs(center.x - point.x);
  const y = Math.abs(center.y - point.y);
  return x < radius && y < radius && Math.sqrt(x * x + y * y) < radius;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (Object.hasOwn(obj, str[i])) obj[str[i]] += 1;
    else obj[str[i]] = 1;
  }
  ...
  const sum = String(ccn)
    ...
    .reduce((ccl, item, ind) => {
      return (
        ...
        (ind % 2 === 1
          ? +item
          : +String(+item * 2)[0] + (+String(+item * 2)[1] || 0))
      );
    }, 0);
  return (10 - (sum % 10)) % 10 === +String(ccn).slice(-1);
}
